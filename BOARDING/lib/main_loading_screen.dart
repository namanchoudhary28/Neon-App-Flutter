...
class _LoadDataState extends State<LoadData> {
  ...
  List list_about,
      ...
      list_projects,
      list_skills,
      list_communications /*, list_badges*/,
      list_blogs,
      list_education,
      list_certifications;
  bool got = false;
  Future<String> getData() async {
    ...
    var response1 = await http.get('http://192.168.1.9:8000/userinfo', headers: {
      'Accept': 'application/json',
      'Authorization': 'Token $token',
    });

    var response2 = await http.get('http://192.168.1.9:8000/hobbies', headers: {
      'Accept': 'application/json',
      'Authorization': 'Token $token',
    });

    var response3 = await http.get('http://192.168.1.9:8000/project/0', headers: {
      'Accept': 'application/json',
      'Authorization': 'Token $token',
    });
    ...
    var response5 = await http.get('http://192.168.1.9:8000/skills', headers: {
      'Accept': 'application/json',
      'Authorization': 'Token $token',
    });
    var response6 =
    ...
    var response7 = await http.get('http://192.168.1.9:8000/blog', headers: {
      'Accept': 'application/json',
      'Authorization': 'Token $token',
    });
    var response8 = await http.get('http://192.168.1.9:8000/education', headers: {
      'Accept': 'application/json',
      'Authorization': 'Token $token',
    });
    var response9 =
        await http.get('http://192.168.1.9:8000/certification', headers: {
      'Accept': 'application/json',
      'Authorization': 'Token $token',
    });
    ...
    return SafeArea(
      ...
      body: Column(
        ...
        children: <Widget>[
          ...
          Loading(indicator: BallSpinFadeLoaderIndicator(), size: 50.0)
        ],
      ),
    ));
  }
